...
if __name__ == '__main__':
    ...
    with open(out_csv, 'w', newline='') as csvfile, open(error_log, 'w') as errfile:
        ...
        for ligand_sdf in ligand_files:
            ...
            try:
                # Try normal processing, if fails, try to fix SDF and process again
                sdf_to_pdbqt_ligand_meeko_rdkit(ligand_sdf, ligand_pdbqt, fixed_dir=fixed_sdf_dir)
                smiles = extract_smiles(ligand_sdf)
                # QVina may fail with non-zero exit status if ligand/receptor is problematic
                run_qvina(qvina_path, receptor_pdbqt, ligand_pdbqt, center, size, output_pdbqt, log_file)
                affinity = extract_affinity(log_file)
                writer.writerow([ligand_name, smiles, affinity])
                print(f"{ligand_name}: Affinity={affinity}, SMILES={smiles}")
